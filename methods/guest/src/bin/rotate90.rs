...
fn main() {
    ...
    let rotate90_image = img.rotate90();

    let mut rotate90_image_output: Vec<u8> = Vec::new();

    rotate90_image
        .write_to(&mut Cursor::new(&mut rotate90_image_output), img_format)
        .expect("Failed to write grayscale image");

    let output = Output {
        image: rotate90_image_output,
        hash: input.hash,
    };
